...
#include "libmedia_codec/vcm_encoded_frame.h"
...
void VCMEncodedFrame::CopyCodecSpecific(const libmedia_transfer_protocol::RTPVideoHeader* header) {
  if (header) {
    switch (header->codec) {
      case kVideoCodecVP8: {
        const auto& vp8_header =
            absl::get<webrtc::RTPVideoHeaderVP8>(header->video_type_header);
      ...
      case kVideoCodecVP9: {
        const auto& vp9_header =
            absl::get<webrtc::RTPVideoHeaderVP9>(header->video_type_header);
        ...
        if (vp9_header.spatial_idx != webrtc::kNoSpatialIdx) {
          _codecSpecificInfo.codecSpecific.VP9.inter_layer_predicted =
              vp9_header.inter_layer_predicted;
          SetSpatialIndex(vp9_header.spatial_idx);
        }
        if (vp9_header.gof_idx != webrtc::kNoGofIdx) {
