...
//	FILE *fp = fopen("get_rgb.rgb", "wb");
//	fwrite(pRGBData, 1, sizeof(pRGBData), fp);
//	fclose(fp);
//#if 1
...
//		return 1;
//	}
//	simplest_rgb24_to_yuv420("get_rgb.rgb", unWidth, unHeight, 1, "chenli.yuv");
//	free(pRGBData);
//#endif // 1
//
//
...
int BmptoRGB(char *filename, int *width/*out*/, int *heith/*out*/, unsigned char **rgboutdata/*out*/) {

	FILE *fp = NULL, *yuv420p = NULL;
	int i, j;

	if (!(fp = fopen(filename, "rb"))) 
		return -1;
	
	unsigned char *rgb24 = *rgboutdata;
	//头信息的结构体的大小 14个字节
	...
	//文件类型
	if (bmpheader.bfType != (WORD)0x4D42) {
		printf("文件类型:%d\n", bmpheader.bfType);
		return -2;
	}
	...
	printf("bmp-->data====%d\n", bmpheader.bfSize);
	//求出位图数据大小
	rgbdata = bmpheader.bfSize - (sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER));


	printf("位图大小:%d\n", rgbdata);

	//开辟rgb数据的内存
	rgb24 = (unsigned char *)malloc(sizeof(unsigned char) * rgbdata);

	memset(rgb24, 0x00, rgbdata);
	//printf("rgb24--%s---\n", rgb24);
	//====================width heith===================
	fread(&bmpdata, 1, sizeof(BITMAPINFOHEADER), fp);

	printf("bmp:widht====%d, heigth===%d\n", bmpdata.biWidth, bmpdata.biHeight);

	printf("rgb:%d\n", bmpdata.biSizeImage);

	//读取数据bmp中rgb数据
	int size = fread(rgb24, 1, rgbdata, fp);
	printf("size = %d, rgb24 --%s---\n", size, rgb24);
	//改变'R'和'B'的位置 
	for (j = bmpdata.biHeight; j > 0; j--) {
		for (i = 0; i< bmpdata.biWidth; i++) {
			char temp = rgb24[(j*bmpdata.biWidth + i) * 3 + 2];
			rgb24[(j*bmpdata.biWidth + i) * 3 + 2] = rgb24[(j*bmpdata.biWidth + i) * 3 + 0];
			rgb24[(j*bmpdata.biWidth + i) * 3 + 0] = temp;
		}
	}
	
	
	char rgbyuv[] = "./hlzdfkgdfskjl.rgb";
	//rgb写到文件中if (!(fp = fopen(filename, "rb")))
	if (!(yuv420p = fopen(rgbyuv, "wb+"))) {
		printf("yuv close\n");
		return -1;
	}
	
	fwrite(rgb24, 1, bmpdata.biHeight * bmpdata.biWidth * 3, yuv420p);
	
	*width = bmpdata.biWidth, *heith = bmpdata.biHeight;
...
int RgbtoYuv420p(unsigned char *rgbdata/*in*/, int width, int heith, unsigned char *yuvBuf) {

	unsigned char*ptrY, *ptrU, *ptrV, *ptrRGB;
	memset(yuvBuf, 0, width*heith * 3 / 2);
	ptrY = yuvBuf;
	ptrU = yuvBuf + width * heith;
	ptrV = ptrU + (width * heith * 1 / 4);
	unsigned char y, u, v, r, g, b;
...
int rgb24_to_yuv420(unsigned char *rgboutdata, int width, int heigth, char *outdata) {

	FILE *fp1 = fopen(outdata, "wb+");

	unsigned char *pic_yuv420 = (unsigned char *)malloc(width * heigth * 3 / 2);

	for (int i = 0; i<1; i++) {
		//fread(pic_rgb24, 1, w*h * 3, fp);
		RgbtoYuv420p(rgboutdata, width,  heigth, pic_yuv420);
		fwrite(pic_yuv420, 1, width * heigth * 3 / 2, fp1);
	}
...
int main(int argc, char* argv[])
{
	...
	int width, heith;
	unsigned char *rgboutdata;
	BmptoRGB(filename, &width/*out*/, &heith/*out*/, &rgboutdata/*out*/);

	//rgb24_to_yuv420(rgboutdata, width, heith, "samples_400_400.yuv");
