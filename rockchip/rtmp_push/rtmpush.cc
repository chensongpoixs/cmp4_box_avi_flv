...
//	rtc::Thread::Start();
	...
	m_thead = std::thread(&AnyRtmpPush::Run, this);
}
...
//	rtc::Thread::SleepMs(100);
	...
	{
		std::lock_guard<std::mutex> l(cs_rtmp_);
		if (rtmp_) {
...
void AnyRtmpPush::SetAacData(uint8_t* pData, int nLen, uint32_t ts)
{
	...
	pdata->_dts = ts;
	std::lock_guard<std::mutex> l( cs_list_enc_);
	lst_enc_data_.push_back(pdata);
	printf("lst_enc_data_  = %u\n", lst_enc_data_.size());
}
...
void AnyRtmpPush::DoSendData()
{
	...
	//* Statics
	//if(stat_time_.time_since_epoch() <= std::chrono::system_clock::now().time_since_epoch())
	{
		//stat_time_ = (std::chrono::system_clock::now() + std::chrono::milliseconds(1000));
		uint32_t delayMs = 0;
