...
SimulcastRateAllocator::SimulcastRateAllocator(const VideoCodec& codec)
    : codec_(codec),
      stable_rate_settings_(webrtc::StableTargetRateExperiment::ParseFromFieldTrials()),
      rate_control_settings_(webrtc::RateControlSettings::ParseFromFieldTrials()),
...
VideoBitrateAllocation SimulcastRateAllocator::Allocate(
  ...
  webrtc::DataRate stable_rate = parameters.total_bitrate;
...
void SimulcastRateAllocator::DistributeAllocationToSimulcastLayers(
    ...
    VideoBitrateAllocation* allocated_bitrates) {
  webrtc::DataRate left_in_total_allocation = total_bitrate;
  webrtc::DataRate left_in_stable_allocation = stable_bitrate;

  if (codec_.maxBitrate) {
	  webrtc::DataRate max_rate = webrtc::DataRate::KilobitsPerSec(codec_.maxBitrate);
  ...
  if (codec_.numberOfSimulcastStreams == 0) {
    ...
    if (codec_.active) {
      allocated_bitrates->SetBitrate(
          0, 0,
          std::max(webrtc::DataRate::KilobitsPerSec(codec_.minBitrate),
  ...
  for (; active_layer < codec_.numberOfSimulcastStreams; ++active_layer) {
	...
	webrtc::DataRate min_bitrate = webrtc::DataRate::KilobitsPerSec(stream.minBitrate);
	webrtc::DataRate target_bitrate = webrtc::DataRate::KilobitsPerSec(stream.targetBitrate);
	...
	webrtc::DataRate layer_rate = std::min(left_in_total_allocation, target_bitrate);
  ...
  if (left_in_total_allocation > webrtc::DataRate::Zero()) {
    const SpatialLayer& stream = codec_.simulcastStream[top_active_layer];
	webrtc::DataRate initial_layer_rate = webrtc::DataRate::BitsPerSec(
        allocated_bitrates->GetSpatialLayerSum(top_active_layer));
	webrtc::DataRate additional_allocation = std::min(
        left_in_total_allocation,
		webrtc::DataRate::KilobitsPerSec(stream.maxBitrate) - initial_layer_rate);
    allocated_bitrates->SetBitrate(
        top_active_layer, 0,
