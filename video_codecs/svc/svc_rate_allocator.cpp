...
#include "libmedia_codec/video_codecs/svc/create_scalability_structure.h"
...
std::vector<webrtc::DataRate> AdjustAndVerify(
    const VideoCodec& codec,
    size_t first_active_layer,
    const std::vector<webrtc::DataRate>& spatial_layer_rates) {
  std::vector<webrtc::DataRate> adjusted_spatial_layer_rates;
  ...
  webrtc::DataRate excess_rate = webrtc::DataRate::Zero();
  for (size_t sl_idx = 0; sl_idx < spatial_layer_rates.size(); ++sl_idx) {
	  webrtc::DataRate min_rate = webrtc::DataRate::KilobitsPerSec(
        codec.spatialLayers[first_active_layer + sl_idx].minBitrate);
	  webrtc::DataRate max_rate = webrtc::DataRate::KilobitsPerSec(
        codec.spatialLayers[first_active_layer + sl_idx].maxBitrate);

	  webrtc::DataRate layer_rate = spatial_layer_rates[sl_idx] + excess_rate;
    ...
    if (layer_rate <= max_rate) {
      excess_rate = webrtc::DataRate::Zero();
...
static std::vector<webrtc::DataRate> SplitBitrate(size_t num_layers,
	webrtc::DataRate total_bitrate,
                                          float rate_scaling_factor) {
  std::vector<webrtc::DataRate> bitrates;
  ...
  const webrtc::DataRate sum =
      std::accumulate(bitrates.begin(), bitrates.end(), webrtc::DataRate::Zero());
...
webrtc::DataRate FindLayerTogglingThreshold(const VideoCodec& codec,
                                    size_t first_active_layer,
                                    size_t num_active_layers) {
  if (num_active_layers == 1) {
    return webrtc::DataRate::KilobitsPerSec(codec.spatialLayers[0].minBitrate);
  }

  if (codec.mode == VideoCodecMode::kRealtimeVideo) {
	  webrtc::DataRate lower_bound = webrtc::DataRate::Zero();
	  webrtc::DataRate upper_bound = webrtc::DataRate::Zero();
    if (num_active_layers > 1) {
      for (size_t i = 0; i < num_active_layers - 1; ++i) {
        lower_bound += webrtc::DataRate::KilobitsPerSec(
            codec.spatialLayers[first_active_layer + i].minBitrate);
        upper_bound += webrtc::DataRate::KilobitsPerSec(
    ...
    // layers respectively.
    while (upper_bound - lower_bound > webrtc::DataRate::BitsPerSec(1)) {
		webrtc::DataRate try_rate = (lower_bound + upper_bound) / 2;
    ...
    return upper_bound;
	  ...
	  webrtc::DataRate toggling_rate = webrtc::DataRate::Zero();
    for (size_t i = 0; i < num_active_layers - 1; ++i) {
      toggling_rate += webrtc::DataRate::KilobitsPerSec(
...
SvcRateAllocator::SvcRateAllocator(const VideoCodec& codec)
    : codec_(codec),
      ...
      experiment_settings_(webrtc::StableTargetRateExperiment::ParseFromFieldTrials()),
...
VideoBitrateAllocation SvcRateAllocator::Allocate(
	...
	webrtc::DataRate total_bitrate = parameters.total_bitrate;
  if (codec_.maxBitrate != 0) {
    total_bitrate =
        std::min(total_bitrate, webrtc::DataRate::KilobitsPerSec(codec_.maxBitrate));
  }
  ...
  if (experiment_settings_.IsEnabled() &&
      parameters.stable_bitrate > webrtc::DataRate::Zero()) {
...
VideoBitrateAllocation SvcRateAllocator::GetAllocationNormalVideo(
  ...
  std::vector<webrtc::DataRate> spatial_layer_rates;
  if (num_spatial_layers == 0) {
  ...
  for (size_t sl_idx = 0; sl_idx < num_spatial_layers; ++sl_idx) {
    std::vector<webrtc::DataRate> temporal_layer_rates =
...
VideoBitrateAllocation SvcRateAllocator::GetAllocationScreenSharing(
  ...
  webrtc::DataRate allocated_rate = webrtc::DataRate::Zero();
  webrtc::DataRate top_layer_rate = webrtc::DataRate::Zero();
  size_t sl_idx;
  for (sl_idx = first_active_layer;
       sl_idx < first_active_layer + num_spatial_layers; ++sl_idx) {
    const webrtc::DataRate min_rate =
		webrtc::DataRate::KilobitsPerSec(codec_.spatialLayers[sl_idx].minBitrate);
    const webrtc::DataRate target_rate =
		webrtc::DataRate::KilobitsPerSec(codec_.spatialLayers[sl_idx].targetBitrate);
  ...
  if (sl_idx > 0 && total_bitrate - allocated_rate > webrtc::DataRate::Zero()) {
    // Add leftover to the last allocated layer.
    top_layer_rate = std::min(
        top_layer_rate + (total_bitrate - allocated_rate),
		webrtc::DataRate::KilobitsPerSec(codec_.spatialLayers[sl_idx - 1].maxBitrate));
...
size_t SvcRateAllocator::FindNumEnabledLayers(webrtc::DataRate target_rate) const {
  if (cumulative_layer_start_bitrates_.empty()) {
    return 0;
  }

  size_t num_enabled_layers = 0;
  for (webrtc::DataRate start_rate : cumulative_layer_start_bitrates_) {
...
webrtc::DataRate SvcRateAllocator::GetMaxBitrate(const VideoCodec& codec) {
  const NumLayers num_layers = GetNumLayers(codec);
  const ActiveSpatialLayers active_layers =
      GetActiveSpatialLayers(codec, num_layers.spatial);

  webrtc::DataRate max_bitrate = webrtc::DataRate::Zero();
  for (size_t sl_idx = 0; sl_idx < active_layers.num; ++sl_idx) {
    max_bitrate += webrtc::DataRate::KilobitsPerSec(
        codec.spatialLayers[active_layers.first + sl_idx].maxBitrate);
  }

  if (codec.maxBitrate != 0) {
    max_bitrate =
        std::min(max_bitrate, webrtc::DataRate::KilobitsPerSec(codec.maxBitrate));
  }

  return max_bitrate;
}

webrtc::DataRate SvcRateAllocator::GetPaddingBitrate(const VideoCodec& codec) {
  auto start_bitrate = GetLayerStartBitrates(codec);
  if (start_bitrate.empty()) {
    return webrtc::DataRate::Zero();  // All layers are deactivated.
  }

  return start_bitrate.back();
}

absl::InlinedVector<webrtc::DataRate, kMaxSpatialLayers>
SvcRateAllocator::GetLayerStartBitrates(const VideoCodec& codec) {
  absl::InlinedVector<webrtc::DataRate, kMaxSpatialLayers> start_bitrates;
  const NumLayers num_layers = GetNumLayers(codec);
  const ActiveSpatialLayers active_layers =
      GetActiveSpatialLayers(codec, num_layers.spatial);
  webrtc::DataRate last_rate = webrtc::DataRate::Zero();
  for (size_t i = 1; i <= active_layers.num; ++i) {
	  webrtc::DataRate layer_toggling_rate =
